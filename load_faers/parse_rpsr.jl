# Parse required fields from RPSR files
# 22 Feb, 2017

# Fields to capture:
# isr, primaryid, caseid, year, qtr, rpsr_cod
  ...
  for ln in eachline(rpsrFile)
    ...
    if !(ismatch(r"ISR", split(ln, "\$")[1]))
      if length(split(ln, "\$")) >= 2
        ...
        year      = "20$yr"
        qtr       = qtr
        rpsr_cod  = strip(split(ln, "\$")[2])
        write(ofile, "$isr\$$primaryid\$$caseid\$$year\$$qtr\$$rpsr_cod\n")
      end
    end
  end
end
...
oFile = open("./final_data/RPSR_FILE.txt","w")
write(oFile, "isr\$primaryid\$caseid\$year\$qtr\$rpsr_cod\n")

for file in readdir(dir)
  if ismatch(r"(?i)RPSR[0-9]{2}Q[0-9]\.TXT", file)
    m = match(r"(?i)RPSR([0-9]{2})Q([0-9])\.TXT", file)
    year = m.captures[1]
    qtr  = m.captures[2]
    #println(year)
